...
int main() {
    ...
    linear_allocator_init(&test_allocator, test_memory_block, sizeof(test_memory_block));

    int* test_int_array = (int*)linear_allocator_allocate(&test_allocator, 10 * sizeof(int));
    ...
    void* test_failed_allocation = linear_allocator_allocate(&test_allocator, 2000);
    assert(test_failed_allocation == NULL && "Allocation should fail (not enough memory)");
    ...
    assert(test_zero_allocation == NULL && "Zero-size allocation should return NULL");
